...
<dl class="dl-horizontal">
  ...
  <dd class="col-sm-6"><label id="note_id"></label></dd>
  <dt class="col-sm-6"><label>Bulletin Number</label></dt>
  <dd class="col-sm-6"><label id="note_number"></label></dd>
<dl>
...
$('document').ready(function(){
    ...
    $.ajax({
      type    : 'POST',
      url     : '../../controllers/notes/get_by_id.php',
      data    : { "note_id" : note_id },
      success : function(arr) {
                  var response = jQuery.parseJSON(arr);
                  $('#note_id').text(response[0].note_id);
                  $('#note_number').text(response[0].note_number);
                }
    }, 'json');
});
